...
export async function sendMessage(message: string) {
    ...
    if (selectedChat.session) {
        ...
        try {
                        ...
                        if (trimmed.startsWith('data:')) {
                            ...
                            try {
                                ...
                                if (data.choices && data.choices.length > 0) {
                                    ...
                                    if (delta.content) {
                                        // 实时更新助手消息内容
                                        selectedChat.session.messages[assistantMessageIndex].content += delta.content;
                                    }
                                }
                            } catch (e) {
            ...
            // 处理错误，可能需要显示错误消息给用户
            selectedChat.session.messages[assistantMessageIndex].content = `Error: ${(error as Error).message}`;
        }
    }
}
