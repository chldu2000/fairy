...
export async function sendMessage(message: string) {
  ...
  // 安全检查：确保 session 存在
  if (!selectedChat.session) {
    console.warn("sendMessage called without active session");
    return;
  }

  // 安全检查：确保有选中的 provider
  const provider = providers.get(preferences.provider);
  if (!provider) {
    console.warn("sendMessage called without selected provider");
    return;
  }
  ...
  const targetEndpoint = provider.endpoint || providerApiEndpoints[provider.apiType].endpoint;
  const body = {
